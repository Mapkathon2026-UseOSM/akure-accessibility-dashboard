...
with col1:
    lga_options = available_lgas + (["Both (compare)"] if len(available_lgas) == 2 else [])
    # Default to "Both (compare)" when it's actually an available option
    # (i.e. both LGAs loaded successfully); fall back to the first LGA
    # otherwise, since "Both (compare)" won't exist in lga_options at all
    # if only one LGA's data is present.
    default_lga_index = lga_options.index("Both (compare)") if "Both (compare)" in lga_options else 0
    lga_choice = st.selectbox("Study area", lga_options, index=default_lga_index)
with col2:
...
with col4:
    ...
    default_basemap_index = basemap_names.index("Streets (OpenStreetMap)")
    basemap_choice = st.selectbox("Basemap", basemap_names, index=default_basemap_index)
with col5:
