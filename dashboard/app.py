...
st.markdown(
    ...
    "cartographic reference standard. Generated by Notebook 03, Section 6.2; regenerate there "
    "after re-running the analysis to refresh what's shown here."
)
...
if not static_files_found:
    ...
    for lga in static_scope_lgas:
        ...
        for category in category_order:
            ...
            with st.expander(f"{category} ({len(cat_files)})", expanded=(category == "Access deficit")):
                cols = st.columns(min(3, len(cat_files)))
    ...
    if os.path.exists(zip_path):
        ...
        st.caption(
            "No bundled ZIP found yet; run Notebook 03, Section 6.3 to produce "
            f"`{zip_path}` for a one-click download of every figure."
        )
