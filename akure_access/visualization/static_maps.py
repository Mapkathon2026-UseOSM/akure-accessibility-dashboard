...
from akure_access.insights import (
    describe_deficit_map as _describe_deficit_map,
    describe_continuous_map as _describe_continuous_map,
    describe_completeness_map as _describe_completeness_map,
    describe_mode_comparison_chart as _describe_mode_comparison_chart,
)
...
def generate_all_static_outputs(
    ...
    Returns a dict: {"print": [...], "web": [...], "captions": {...}}.
    `captions` maps each print-tier filename to a caption string
    (description, legend explanation, and inferred result, generated by
    akure_access.insights from this same grid_gdf), the SAME caption is
    also written to `out_dir/captions.json` so a consumer that only has
    the file paths (not this function's return value), such as
    dashboard/app.py reading files back off disk later, can still look
    up the right caption for each image without needing to regenerate
    it or hand-write it separately.
    """
    ...
    for mode in modes:
        ...
        produced["captions"][fname] = _describe_deficit_map(grid_gdf, lga_name, mode)
        ...
        for service, label in (("health", "Health"), ("education", "Education")):
            ...
            if col in grid_gdf.columns and grid_gdf[col].notna().any():
                ...
                produced["captions"][fname] = _describe_continuous_map(grid_gdf, lga_name, mode, service)
    ...
    for service in ("health", "education"):
        ...
        if flag_col in grid_gdf.columns:
            ...
            produced["captions"][fname] = _describe_completeness_map(grid_gdf, lga_name, service)
    ...
    if mode_stats:
        ...
        produced["captions"][fname] = _describe_mode_comparison_chart(mode_stats, lga_name)

    captions_path = os.path.join(out_dir, "captions.json")
    with open(captions_path, "w", encoding="utf-8") as f:
        json.dump(produced["captions"], f, indent=2)
